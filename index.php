...
// THIS PART IS RESPONSIBLE FOR GETTING HTTP HEADERS FROM A REQUEST. IT GETS AUTHORIZATION HEADER MOST IMPORTANTLY
if(isset($_POST['nimabu'])){
...
for ($i = 0; $i < mysqli_num_rows($result); $i++){
	echo ($i>0?',':'');
	$tempres = mysqli_fetch_object($result);
	$tempres->liked = true;
...
function Somedata($conn, $query){
	...
	//THIS PART IS FOR WORKING WITH LIKE / NOT LIKE IKONS. THE SERVER SENDS STATUSES OF LIKED PLACES
	echo '[';
	for ($i = 0; $i < mysqli_num_rows($result); $i++){
		echo ($i>0?',':'');
		$tempres = mysqli_fetch_object($result);
		
		$tempres->yoqtir = false; //THIS LINE ADD A PROPERTY TO A PLACE. FOR EXAMPLE FALSE SETS A PLACE TO UNLIKED
		echo json_encode($tempres);
		//echo json_encode(mysqli_fetch_object($result));
	}
	echo ']';
